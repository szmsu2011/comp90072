...
```{r sszc, fig.height = 3.8, fig.cap = "Time series of chest movement (top) and heart rates (bottom) at 1Hz for 1 minute. Blue: naive zero-crossing gives a count of 11.5 and 6 breaths; red: smoothing spline crossing gives a count of 19.5 and 16.5 breaths. We can clearly see that smoothing spline performs much better in counting breaths."}
```
...
The trained model implied that one's heart rate is useful for inferring the rate of breath, and the results are statistically significant based on the regularisation result. If there is null relationship between the heart rate and rate of breath, the trained model should have returned a *least-complex fit*, such the $L_2$ loss function (cross-validated MSE) is minimised at large $\lambda$ and is monotonically decreasing. Hence, the obvious inverted bell curve exhibited by the regularised $L_2$ loss with one minimum gives evidence against a *null-model fit*.
