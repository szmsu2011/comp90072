...
\begin{algorithmic}
\STATE Take filtered $\mathbf{z} \in \mathbb{R}^n$ as the 100-Hz input
\STATE Set moving threshold $\mathbf{c}$ as half of rolling maximum
\FOR{$t = 1, ..., 100$}
\STATE $c_t \leftarrow +\infty$
\ENDFOR
\FOR{$t = 101, ..., n$}
...
\FOR{$t \in \{\mathbf{p}\}$}
\STATE $p_{p = t} \leftarrow \text{argmax}_t\{z_{t - 4}, ..., z_t, ... z_{t + 4}\}$
\ENDFOR
\RETURN $\mathbf{p}$ as the timestamps of the R peak
\end{algorithmic}
\end{algorithm}
