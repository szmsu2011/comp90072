...
Before starting, you will need to covert the raw `.dat` data files into `.csv` files as follows:
...
Note that if you are running R on a Unix-like system (e.g., macOS, Linux), set argument `n_core` up to the number of available CPU cores on your machine to speed up the process (via command line only).
